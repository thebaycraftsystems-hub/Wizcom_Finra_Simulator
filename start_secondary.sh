...
pwd
kill -9 `ps -eafw | grep 'WizFixSim_Praditha_secondary' | grep -v grep | awk '{print $2}'| xargs`
#java -Dname=WizFixSim_Praditha_secondary -jar fix-simulator.jar secondary quickfixj-server-secondary.cfg &

java -Dname=WizFixSim_Praditha_secondary -Dquickfixj.config=file:./quickfixj-server-secondary.cfg -jar fix-simulator.jar  quickfixj-server-secondary.cfg &
