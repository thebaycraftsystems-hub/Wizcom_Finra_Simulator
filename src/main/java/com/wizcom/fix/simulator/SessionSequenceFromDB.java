 ...
 * for a given SessionID. Used on Logon to align session state with DB and detect missing sequences.
...
public class SessionSequenceFromDB {
	...
	/**
	 * Fetches the max sequence numbers from the database for this session, only when the row is for the current date.
	 * Uses creation_time to ensure we use sequence state for today's session day (in sessionDateZone), not a previous day.
	 * @param sessionID QuickFIX/J session (acceptor view: we are sender, initiator is target)
	 * @return SessionSequence with incoming_seqnum and outgoing_seqnum, or null if row not found, not for current date, or error
	 */
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					...
					if (rowDate == null || !rowDate.equals(today)) {
						log.debug("Session sequence row for {} is not for current date (creation_time date={}, today={}); not using for max seq.",
								sessionID, rowDate, today);
						return null;
					}
					// FIX sequence numbers must be >= 1; never use 0 from DB
