...
//    private DataDictionary dictionary;
	...
	public void toApp(Message msg, SessionID arg1) throws DoNotSend {
		...
		/**
		 * TradeCaptureReport resTrdCapRpt = (TradeCaptureReport) msg; try { quickfix.fix44.TradeCaptureReport.NoSides sidesGroup = new quickfix.fix44.TradeCaptureReport.NoSides();
		 * 
		 * log.info("NoSide :: ["+resTrdCapRpt.getNoSides().getValue()+"]"); for(int grpIndex = 1; grpIndex<= resTrdCapRpt.getNoSides().getValue(); grpIndex += 1) { // get 'grpIndex' sidesGroup
		 * resTrdCapRpt.getGroup(grpIndex, sidesGroup);
		 * 
		 * log.info("Side Value ["+sidesGroup.getSide().getValue()+"]");
		 * 
		 * // NoPartyIds subgroup quickfix.fix44.TradeCaptureReport.NoSides.NoPartyIDs partyIdsGroup = new quickfix.fix44.TradeCaptureReport.NoSides.NoPartyIDs();
		 * 
		 * log.info("NoPartyIDs :: ["+sidesGroup.getParties().getNoPartyIDs().getValue()+"]"); for(int partyIndex =1; partyIndex <= sidesGroup.getParties().getNoPartyIDs().getValue(); partyIndex +=1 ) {
		 * 
		 * // get 'partyIndex' partyIdsGroup from sidesGroup sidesGroup.getGroup(partyIndex, partyIdsGroup); log.info("Before :: PartyID ["+partyIdsGroup.getPartyID().getValue()+"] PartyRole
		 * ["+partyIdsGroup.getPartyRole().getValue()+"]");
		 * 
		 * if(17 == partyIdsGroup.getPartyRole().getValue()) { // sidesGroup.removeGroup(partyIdsGroup); // partyIdsGroup.set(new PartyID("JPMS")); log.info("After :: PartyID
		 * ["+partyIdsGroup.getPartyID().getValue()+"] PartyRole ["+partyIdsGroup.getPartyRole().getValue()+"]"); //sidesGroup.addGroup(partyIdsGroup); break; } } } } catch(Exception e) {
		 * 
		 * }
		 **/
	}
	...
	public synchronized void onMessageHold(TradeCaptureReport msg, SessionID sessionID) throws FieldNotFound, SessionNotFound, ConfigError, FieldConvertError {
		...
		if (traceNotAvailable) {
			...
			if (0 == msg.getTradeReportTransType().getValue()) {
				...
				/*
				 * if("SP".equalsIgnoreCase( msg.getHeader().getField(new TargetSubID()).getValue())) { processingTradeEntry(msg, sessionID, "SPEN"); }else if("CA".equalsIgnoreCase( msg.getHeader().getField(new
				 * TargetSubID()).getValue())) { processingTradeEntry(msg, sessionID, "CAEN"); }else if("TS".equalsIgnoreCase( msg.getHeader().getField(new TargetSubID()).getValue())) { processingTradeEntry(msg,
				 * sessionID, "TSEN"); }
				 */
				...
				/*
				 * if("SP".equalsIgnoreCase( msg.getHeader().getField(new TargetSubID()).getValue())) { if(!msg.isSetField(22009)) { processingTradeCancel(msg, sessionID, "SPCX"); }else { processingTradeReversal(msg,
				 * sessionID, "SPHX"); } }else if("CA".equalsIgnoreCase( msg.getHeader().getField(new TargetSubID()).getValue())) { if(!msg.isSetField(22009)) { processingTradeCancel(msg, sessionID, "CACX"); }else {
				 * processingTradeReversal(msg, sessionID, "CAHX"); } }else if("TS".equalsIgnoreCase( msg.getHeader().getField(new TargetSubID()).getValue())) { processingTradeCancel(msg, sessionID, "TSCX"); }
				 */
			} // MODIFY
	...
	private void processingREJ(TradeCaptureReport reqTrdCapRpt, SessionID sessionID, String securityType) throws FieldNotFound, SessionNotFound {
		...
		/*
		 * Instrument instrument = new Instrument(); instrument.set(new SecurityID(reqTrdCapRpt.getSecurityID().getValue())); instrument.set(new
		 * SecurityIDSource(reqTrdCapRpt.getSecurityIDSource().getValue())); resTrdCapRpt.set(instrument);
		 */
	...
	private void processingTradeEntry(TradeCaptureReport reqTrdCapRpt, SessionID sessionID, String securityType) {
		...
		try {
			...
			if (777 == reqTrdCapRpt.getLastPx().getValue()) {
				...
				processingAllege(resTrdCapRpt, sessionID, securityType, securityType + "AL");
			}
	...
	// CANCEL
	private void processingTradeCancel(TradeCaptureReport reqTrdCapRpt, SessionID sessionID, String securityType) throws FieldNotFound, SessionNotFound {
		TradeCaptureReport resTrdCapRpt = new TradeCaptureReport();
		...
		try {
			...
			resTrdCapRpt.setField(new TradeReportID(getTrdRptID())); // TradeReportID
			// resTrdCapRpt.setField(new StringField(22011, getControlDate()));//ControlDate ---- need to change
			...
			/**
			 * Instrument instrument = new Instrument(); resTrdCapRpt.get(instrument); instrument.set(new NoSecurityAltID(1)); TradeCaptureReport.NoSecurityAltID altIDGrp = new
			 * TradeCaptureReport.NoSecurityAltID(); altIDGrp.set(new SecurityAltIDSource(FinraSecurityIDSource.SYMBOL)); altIDGrp.set(new SecurityAltID("Need2Generate")); instrument.addGroup(altIDGrp);
			 * resTrdCapRpt.set(instrument);
			 **/
	...
	private void processingTradeReversal(TradeCaptureReport reqTrdCapRpt, SessionID sessionID, String securityType) {
		...
		try {
			...
			/*
			 * StringField tradeID = new StringField(1003); resTrdCapRpt.setField(new StringField(1126, reqTrdCapRpt.getField(tradeID).getValue()));// Orginal
			 */
			resTrdCapRpt.setField(new TradeModifier3(TradeModifier3.T));// TradeModifier3
			...
			for (int grpIndex = 1; grpIndex <= resTrdCapRpt.getNoSides().getValue(); grpIndex += 1) {
				...
				for (int partyIndex = 1; partyIndex <= sidesGroup.getParties().getNoPartyIDs().getValue(); partyIndex += 1) {
					...
					if (17 == partyIdsGroup.getPartyRole().getValue()) {
						...
						if (sidesGroup.getSide().getValue() == Character.valueOf('1')) {
							rpidSide = '1';
							cpidSide = '2';
						} else {
							rpidSide = '2';
	...
	/*
	 * private String getFinraControlNo() { return Long.valueOf(System.currentTimeMillis() + (nextID++)).toString().substring(0, 9); }
	 */
