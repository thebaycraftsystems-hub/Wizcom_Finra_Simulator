...
## Wall-clock session timeout (application logic)

Set on each **`[session]`** block that needs it (not required in `[default]`). **WizFixApplication** reads these per `SessionID`; if a session omits them, behavior falls back to `[default]` when those keys exist there, otherwise no timeout / defaults.

| Field | Values | How it is used |
|-------|--------|----------------|
| **SessionTimeoutRequired** | True \| False | If True, after **Logon** a wall-clock timer starts for that session only (traffic does not reset it). `Boolean.parseBoolean`. |
| **SessionTimeoutSeconds** | positive integer | Seconds until timeout fires. If **SessionTimeoutRequired** is True but this is ≤ 0, timeout is disabled for that session. |
| **isLogoutRequiredatSessionTimeout** | True \| False | On timeout: **True** = **Logout**; **False** = log only. If missing in `[default]`, Java default for fallback is **True**. |
| **LogOnRejectRequired** | Y \| N | **WizFixApplication** (`fromAdmin`): per **`[session]`** only. **Y** = reject the initiator’s **Logon** with **RejectLogon** for that session (no Logon response). **N** = normal accept path. If the key is missing on the session, treated as **N**. |
...
| Used by | Settings |
|---------|----------|
| **Simulator (startup, JDBC, shutdown)** | SimulatorRole, LogToFile, LogToDB, LogToScreen, UseJdbcStore, SendLogout_at_Shutdown, SessionTimeoutRequired, SessionTimeoutSeconds, isLogoutRequiredatSessionTimeout, SessionDateZone, JdbcURL, JdbcUser, JdbcPassword, JdbcDriver, JdbcStoreSessionsTableName, JdbcStoreMessagesTableName, JdbcLogIncomingTable, JdbcLogOutgoingTable, JdbcLogEventTable |
